...
echo "/* generates isotope-array[mass,ordnung] */"

echo "kill(isotope_array); "
IFS=$'\n'
...
do
    if (( $COUNTER < $skiplines )); then
        ...
        ordnungszahl=${line:11:3}
        ordnungszahl=`echo $ordnungszahl | sed "s/ //g"`

        name=${line:20:2}
        name=`echo $name | sed "s/ //g"`

        mass="${line:106:3}.${line:110:6}${line:117:6}"
        ...
        if [[ "$masscheck" == "0" && "$namecheck" == "0" ]]; then
            echo "isotope_array[${massenzahl},${ordnungszahl}] :${mass} * %u $"
        fi
    fi
done < "$infile"
